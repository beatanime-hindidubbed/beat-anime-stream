...
export default function VerifyPage() {
  ...
  // If verification disabled or already verified, redirect home
  useEffect(() => {
  ...
  const handleCheckboxVerify = () => {
    if (!checkboxChecked) return;
    setLoading(true);
    // Simple checkbox — verify after a brief delay to feel real
    setTimeout(() => {
      writeVerification({ code: "checkbox" });
      setSuccess(true);
  ...
  const handleVerify = async () => {
    if (settings.verificationMode === "checkbox") {
      handleCheckboxVerify();
      return;
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-background">
        ...
        >
          ...
          <h1 className="font-display text-2xl font-bold text-foreground">Verified!</h1>
          <p className="text-sm text-muted-foreground">Redirecting to Beat Anime...</p>
        </motion.div>
      </div>
    );
  }
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="bg-card border border-border rounded-2xl p-8">
            <div className="text-center mb-8">
              ...
              <h1 className="font-display text-2xl font-bold text-foreground mb-2">Human Verification</h1>
              <p className="text-muted-foreground text-sm">Please verify you're not a robot to continue.</p>
            </div>

            <div className="flex items-center gap-4 p-4 rounded-xl border border-border bg-secondary/30 mb-6">
              <button
              ...
              >
                {checkboxChecked && <CheckCircle className="w-5 h-5" />}
              </button>
              <span className="text-sm text-foreground font-medium">I'm not a robot</span>
              <div className="ml-auto flex flex-col items-end gap-0.5">
                <div className="w-8 h-8 rounded bg-secondary flex items-center justify-center">
                  <Shield className="w-4 h-4 text-muted-foreground" />
                </div>
                <span className="text-[8px] text-muted-foreground">Beat Anime</span>
              </div>
            </div>
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-background">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        <div className="bg-card border border-border rounded-2xl p-8">
          ...
          <a href="https://t.me/Beat_AniStream_hub_bot" target="_blank" rel="noopener noreferrer"
            ...
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i}
                className={`w-11 h-14 rounded-lg border-2 flex items-center justify-center text-xl font-bold font-mono transition-all duration-150 ${
                  code[i] ? "border-primary text-foreground bg-primary/5 scale-105" : "border-border text-muted-foreground/30"
                }`}>
                {code[i] || "·"}
              </div>
            ))}
        ...
        <div className="mt-4 text-center">
          <span className="text-xs text-muted-foreground/50">@Beat_AniStream_hub_bot</span>
        </div>
      </motion.div>
    </div>
  );
